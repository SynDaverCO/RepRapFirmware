...
static constexpr SpiMode spiMode = SpiMode::mode0;
static constexpr uint32_t spiTimeout = 25/*Mutex::TimeoutUnlimited*/;							// timeout while waiting for the SPI bus

// constructor

MAX3421e::MAX3421e(SharedSpiDevice& dev, uint32_t freq, Pin p_csPin, Pin p_int1Pin) noexcept
	: SharedSpiClient(dev, freq, spiMode, p_csPin, false), int1Pin(p_int1Pin) {
}

// write single byte into MAX3421 register
void MAX3421e::regWr(uint8_t reg, uint8_t data) {
	if(Select(spiTimeout)) {
		const uint8_t msg[2] = {uint8_t(reg | 0x02), data};
		WritePacket(msg, 2);
		Deselect();
	}
}
...
uint8_t* MAX3421e::bytesWr(uint8_t reg, uint8_t nbytes, uint8_t *data_p) {
	if(Select(spiTimeout)) {
		uint8_t msg = reg | 0x02;
		WritePacket(&msg, 1);
		WritePacket(data_p, nbytes);
		data_p += nbytes;
		Deselect();
	}
	return data_p;
...
bool MAX3421e::start() {
	...
	if (revision == 0x00 || revision == 0xFF) {
		debugPrintf("Revision register appears incorrect on MAX3421e initialization. Got %d\n", revision);
		return false;
	}

	if (!reset()) {
		debugPrintf("OSCOKIRQ hasn't asserted in time\n");
		return false;
	}
...
uint8_t MAX3421e::Task() {
	return digitalRead(int1Pin) ? 0 : IntHandler();
}
