...
void uhsErrorMessage(uint8_t level, char const * msg, ERROR_TYPE rcode = 0) {
  #ifdef DEBUG_USB_HOST
    Notify(msg, level);
    Notify(PSTR(": "), level);
...
template <class ERROR_TYPE>
void uhsErrorMessage(char const * msg __attribute__((unused)), ERROR_TYPE rcode __attribute__((unused)) = 0) {
  #ifdef DEBUG_USB_HOST
    Notify(msg, 0x80);
    Notify(PSTR(": "), 0x80);
