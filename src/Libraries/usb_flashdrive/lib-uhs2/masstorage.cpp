...
uint8_t BulkOnly::Init(uint8_t parent __attribute__((unused)), uint8_t port __attribute__((unused)), bool lowspeed) {
  ...
  uhsErrorMessage<uint8_t> (PSTR("MaxLUN"), bMaxLUN);
  ...
  for (uint8_t lun = 0; lun <= bMaxLUN; lun++) {
    ...
    if (rcode) {
      uhsErrorMessage<uint8_t> (PSTR("Inquiry"), rcode);
    }
    else {
      #if 0
        printf("LUN %i `", lun);
...
void BulkOnly::EndpointXtract(uint8_t conf, uint8_t iface, uint8_t alt, uint8_t proto __attribute__((unused)), const USB_FD_ENDPOINT_DESCRIPTOR * pep) {
  uhsErrorMessage<uint8_t> (PSTR("Conf.Val"), conf);
  uhsErrorMessage<uint8_t> (PSTR("Iface Num"), iface);
  uhsErrorMessage<uint8_t> (PSTR("Alt.Set"), alt);
...
bool BulkOnly::CheckLUN(uint8_t lun) {
  ...
  uhsErrorMessage<uint8_t> (PSTR(">>>>>>>>>>>>>>>>CAPACITY OK ON LUN"), lun);
  ...
  if (CurrentCapacity[lun] == /*0xFFFFFFFFUL */ 0x01UL || CurrentCapacity[lun] == 0x00UL) {
    ...
    if (CurrentCapacity[lun])
      uhsErrorMessage<uint8_t> (PSTR(">>>>>>>>>>>>>>>>BUGGY FIRMWARE. CAPACITY FAIL ON LUN"), lun);
    return false;
  }
  delay(20);
  Page3F(lun);
...
uint8_t BulkOnly::ClearEpHalt(uint8_t index) {
        ...
        if (ret) {
                uhsErrorMessage<uint8_t> (PSTR("ClearEpHalt"), ret);
                uhsErrorMessage<uint8_t> (PSTR("EP"), ((index == epDataInIndex) ? (0x80 | epInfo[index].epAddr) : epInfo[index].epAddr));
                return ret;
        }
        epInfo[index].bmSndToggle = 0;
        epInfo[index].bmRcvToggle = 0;
...
uint8_t BulkOnly::HandleUsbError(uint8_t error, uint8_t index) {
  ...
  while (error && count) {
    if (error != hrSUCCESS) {
      uhsErrorMessage<uint8_t> (PSTR("USB Error"), error);
      uhsErrorMessage<uint8_t> (PSTR("Index"), index);
    }
    switch (error) {
      // case hrWRONGPID:
...
uint8_t BulkOnly::Transaction(CommandBlockWrapper *pcbw, uint16_t buf_size, void *buf) {
  ...
  uhsErrorMessage<uint32_t> (PSTR("CBW.dCBWTag"), pcbw->dCBWTag);
  ...
  if (ret)
    uhsErrorMessage<uint8_t> (PSTR("============================ CBW"), ret);
  else {
    if (bytes) {
      if (!write) {
      ...
      if (ret) uhsErrorMessage<uint8_t> (PSTR("============================ DAT"), ret);
    }
  }
  ...
  ret = HandleUsbError(usberr, epDataInIndex);
  if (ret) uhsErrorMessage<uint8_t> (PSTR("============================ CSW"), ret);

  if (usberr == hrSUCCESS) {
    if (IsValidCSW(&csw, pcbw)) {
      //uhsErrorMessage<uint32_t> (PSTR("CSW.dCBWTag"), csw.dCSWTag);
      //uhsErrorMessage<uint8_t> (PSTR("bCSWStatus"), csw.bCSWStatus);
      //uhsErrorMessage<uint32_t> (PSTR("dCSWDataResidue"), csw.dCSWDataResidue);
      Notify(PSTR("CSW:\t\tOK\r\n\r\n"), 0x80);
...
uint8_t BulkOnly::HandleSCSIError(uint8_t status) {
  ...
  switch (status) {
    ...
    case 2:
      uhsErrorMessage<uint8_t> (PSTR("Phase Error"), status);
      uhsErrorMessage<uint8_t> (PSTR("LUN"), bTheLUN);
      ResetRecovery();
      return MASS_ERR_GENERAL_SCSI_ERROR;

    case 1:
      uhsErrorMessage<uint8_t> (PSTR("SCSI Error"), status);
      uhsErrorMessage<uint8_t> (PSTR("LUN"), bTheLUN);
      ...
      uhsErrorMessage<uint8_t> (PSTR("Response Code"), rsp.bResponseCode);
      ...
      uhsErrorMessage<uint8_t> (PSTR("Sense Key"), rsp.bmSenseKey);
      uhsErrorMessage<uint8_t> (PSTR("Add Sense Code"), rsp.bAdditionalSenseCode);
      uhsErrorMessage<uint8_t> (PSTR("Add Sense Qual"), rsp.bAdditionalSenseQualifier);
    ...
    default:
      uhsErrorMessage<uint8_t> (PSTR("Gen SCSI Err"), status);
      uhsErrorMessage<uint8_t> (PSTR("LUN"), bTheLUN);
      return status;
  } // switch
}
